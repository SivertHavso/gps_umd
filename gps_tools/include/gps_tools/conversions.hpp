...
#ifndef GPS_TOOLS__CONVERSIONS_HPP_
#define GPS_TOOLS__CONVERSIONS_HPP_
  ...
  char * UTMZone)
{
  ...
  snprintf(UTMZone, 4, "%d%c", ZoneNumber % 100, UTMLetterDesignator(Lat));  // NOLINT
  ...
  double eccFourthPower = eccSquared * eccSquared;
  double eccSixthPower = eccFourthPower * eccSquared;

  M = a *
    ((1 - eccSquared / 4 - 3 * eccFourthPower / 64 - 5 * eccSixthPower / 256) * LatRad -
    (3 * eccSquared / 8 + 3 * eccFourthPower / 32 + 45 * eccSixthPower / 1024) * sin(2 * LatRad) +
    (15 * eccFourthPower / 256 + 45 * eccSixthPower / 1024) * sin(4 * LatRad) -
    (35 * eccFourthPower * eccSquared / 3072) * sin(6 * LatRad));

  UTMEasting = static_cast<double>(k0 * N * (A + (1 - T + C) * A * A * A / 6 +
...
#endif  // GPS_TOOLS__CONVERSIONS_HPP_
